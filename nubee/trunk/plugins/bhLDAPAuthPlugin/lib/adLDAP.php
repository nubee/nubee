...
class adLDAP {
	...
	function __construct($options = array()) {
		...
		if ($this->_ad_username!=NULL && $this->_ad_password!=NULL){
			$this->_bind = @ldap_bind($this->_conn,$this->formatUsername($this->_ad_username),$this->_ad_password);
			if (!$this->_bind){
				if ($this->_use_ssl){
  ...
  function formatUsername($username) {
    return 'uid=' . $username . $this->getBaseDN();
  }

  function getBaseDN() {
    return (($this->_base_dn != '') ? ',' . $this->_base_dn : '');
  }
	...
	function authenticate($username,$password,$prevent_rebind=false){
		...
		//bind as the user
		$this->_bind = @ldap_bind($this->_conn,$this->formatUsername($username),$password);
